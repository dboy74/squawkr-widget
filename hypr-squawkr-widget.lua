-- Squawkr widget panel — keep it on the special:squawkr scratchpad workspace (the launcher
-- toggles that workspace to show/hide the panel instantly), float, size it to the panel content,
-- dock it just below the bar icon (right-justified so it can't run off-screen), and force it
-- fully opaque.
...
-- silently centred instead — so the installer fills the x placeholder in with (monitor_width - 612)
...
o.window("^(chrome-127.0.0.1__index.html-Default)$", { workspace = "special:squawkr" })
